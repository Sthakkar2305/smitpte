...
import { BulkDeleteTasks } from "@/components/dashboard/admin/bulk-delete-tasks";
...
export default function TaskManager({ token }: TaskManagerProps) {
  ...
  const handleCreateStudentSelection = (
    studentId: string,
    checked: boolean
  ): void => {
    if (checked) {
  ...
  const handleEditStudentSelection = (
    studentId: string,
    checked: boolean
  ): void => {
    if (checked) {
          ...
          <div className="flex items-center space-x-2">
            <Checkbox
              ...
              onCheckedChange={(checked: boolean) =>
                handleCreateAssignToAll(checked)
              }
            />
