...
// Replace the old FileObject and FileData interfaces with this single, unified version
interface FileData {
  originalName?: string;
  originalname?: string; // Kept for compatibility with any old data
  filename?: string;
  url?: string;
  publicId?: string;
  size?: number;
...
interface Material {
  ...
  description: string;
  content: string;
  files: FileData[]; // Use the unified interface here
  uploadedBy: {
    name: string;
    _id?: string;
  };
...
const downloadFile = async (file: FileData) => {
  if (!file.url) {
    ...
    alert("This file cannot be downloaded because its URL is missing.");
    return;
  }
  ...
  try {
    ...
    const filename = fileExtension 
      ? `${originalName.split('.')[0]}.${fileExtension}`
      : originalName;
